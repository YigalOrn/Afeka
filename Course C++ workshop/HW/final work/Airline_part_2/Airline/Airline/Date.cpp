...
Date::Date(int d, int m, int y) throw (DateException)
{
	setDate(d, m, y);
}
...
void Date::setDate(int d, int m, int y) throw (DateException)
{
	...
	{
		throw DateException("# The year assigned, has passed #");
	}

	if (m < 1 || 12 < m)
	{
		throw DateException("# Month is not between 1 to 12 #");
	}

	if (getMonthRange(m, y) < d)
	{
		throw DateException("# The day has passed #");
	}
			...
			(d < timeStruct.tm_mday)))
	{
		...
		throw DateException("# The month and day have passed #");
	}
...
const Date& Date::operator+=(int days) throw (DateException)
{
	if (days < 0)
	{
		throw DateException("# Days amount can't be negative  #");
	}
...
int Date::getMonthRange(int m, int y) throw (DateException)
{
	...
	default:
		throw DateException("# Invalid month and/or year values #");
	}
}
