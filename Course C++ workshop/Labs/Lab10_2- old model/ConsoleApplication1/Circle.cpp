...
Circle::Circle(const char* color, int thickness, float radius) throw (CircleException) :Shape(color, thickness)
{
	setRadius(radius);
}

Circle::Circle(iostream& is) : Shape(is)
{
	//already checked in output
	is >> radius;
}
...
void Circle::toOs(iostream& os) const
{
	os << radius;
}
...
void Circle::setRadius(float radius) throw (CircleException)
{
	if (radius < 0)
	{
		//throw CircleException(getThickness(), radius);
	}
	else
	{
		this->radius = radius;
