...
public:
    ...
    bool hasSufficientSavings(const Customer& c, int requestedAmount) const
    {
        return
            c.getTotalSaving()/(double)requestedAmount > LOAN_TO_SAVING_RATIO;
    }
}; // Bank
...
class IMortgage
{
    virtual bool isEligible(const Customer& customer, int requestedAmount) const = 0;
}; // IMortgage

class MortgageFacade : public IMortgage
{
public:
    bool isEligible(const Customer& customer, int requestedAmount) const
    {
		cout << customer << " applies for " << requestedAmount << " loan\n";

        // Check creditworthyness of applicant 
        cout << "Check " << customer << " bank for saving / loan ratio\n";
        if (!bank.hasSufficientSavings(customer, requestedAmount))
        {
